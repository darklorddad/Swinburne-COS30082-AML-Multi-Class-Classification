...
def user_authentication(request: Request) -> Optional[str]:
    """
...
async def load_index(request: Request, token: Optional[str] = Depends(user_authentication)):
    """
    ...
    if AUTOTRAIN_LOCAL:
        if "local" not in _users:
            _users.insert(0, "local")
...
async def handle_form(
    ...
    token: Optional[str] = Depends(user_authentication),
):
    """
    ...
    if autotrain_user != "local" and repo_exists(f"{autotrain_user}/{project_name}", token=token):
        raise HTTPException(
            status_code=409,
    ...
    if len(hub_dataset) == 0:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Dataset preparation failed: {e}")
    else:
